...
namespace REON
{
void TransparentPass::init(const VulkanContext* context, std::vector<VkDescriptorSetLayout> layouts,
                           VkPipelineCache cache)
{
    m_FrameData.resize(context->MAX_FRAMES_IN_FLIGHT);
    m_Layouts = layouts;
    m_PipelineCache = cache;
    context->createCommandPool(m_CommandPool,
                               context->findQueueFamilies(context->getPhysicalDevice()).graphicsFamily.value());
...
void TransparentPass::render(
    const VulkanContext* context, std::shared_ptr<Camera> camera,
        ...
        for (const auto& material : materialFromShader)
        {
            ...
            for (const auto& cmd : material.second)
            {
                ...
                std::vector<VkDescriptorSet> descriptorSets = {
                    globalDescriptorSet, mat->descriptorSets[currentFrame],
                    cmd.owner->objectDescriptorSets[context->getCurrentFrame()]};
                ...
                vkCmdBindDescriptorSets(cameraData.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                        m_GraphicsPipelineLayout, 0, descriptorSets.size(), descriptorSets.data(), 0,
                                        nullptr);
                vkCmdDrawIndexed(cameraData.commandBuffer, static_cast<uint32_t>(cmd.indexCount), 1, cmd.startIndex, 0,
                                 0);
            }
        }
...
void TransparentPass::resize(const VulkanContext* context, std::shared_ptr<Camera> camera,
                             std::vector<VkImageView>& endViews, std::vector<VkImageView> opaqueImageViews,
...
void TransparentPass::createBuffers(const VulkanContext* context, std::shared_ptr<Camera> camera)
{
    ...
    for (size_t i = 0; i < context->getAmountOfSwapChainImages(); i++)
    {
        ...
        m_SwapChainResourcesByCamera[camera][i].frameInfoBuffer = context->createBuffer(createInfo);
    }
}
...
void TransparentPass::createFrameBuffers(const VulkanContext* context, std::shared_ptr<Camera> camera,
                                         ...
                                         std::vector<VkImageView>& depthImageViews)
{
    for (size_t i = 0; i < context->getAmountOfSwapChainImages(); i++)
    {
        ...
        VkResult res = vkCreateFramebuffer(context->getDevice(), &framebufferInfo, nullptr,
    ...
    for (size_t i = 0; i < context->getAmountOfSwapChainImages(); i++)
    {
        ...
        VkResult res = vkCreateFramebuffer(context->getDevice(), &framebufferInfo, nullptr,
                                           &m_SwapChainResourcesByCamera[camera][i].compositeFramebuffer);
        REON_CORE_ASSERT(res == VK_SUCCESS, "Failed to create framebuffer");
    }
}
...
void TransparentPass::createGraphicsPipelines(const VulkanContext* context)
{
    ...
    VkResult res =
        vkCreatePipelineLayout(context->getDevice(), &pipelineLayoutInfo, nullptr, &m_GraphicsPipelineLayout);
    ...
    res = vkCreateGraphicsPipelines(context->getDevice(), m_PipelineCache, 1, &pipelineInfo, nullptr,
                                    &m_GraphicsPipeline);
    ...
    for (auto permutation : prebakePermutations)
    {
        m_PipelineByMaterialPermutation.insert(
            {permutation, createPermutationGraphicsPipeline(context, m_GraphicsPipeline, permutation)});
    }
...
void TransparentPass::createDescriptorSetLayouts(const VulkanContext* context)
{
    ...
    std::array<VkDescriptorSetLayoutBinding, 4> bindings = {opaqueTexLayoutBinding, accumTexLayoutBinding,
                                                            revealTexLayoutBinding, frameInfoLayoutBinding};
    ...
    VkResult res =
        vkCreateDescriptorSetLayout(context->getDevice(), &layoutInfo, nullptr, &m_CompositeDescriptorSetLayout);
    REON_CORE_ASSERT(res == VK_SUCCESS, "Failed to create descriptor set layout");
}

void TransparentPass::createDescriptorSets(const VulkanContext* context, std::shared_ptr<Camera> camera,
                                           std::vector<VkImageView> opaqueImageViews)
{
    std::vector<VkDescriptorSetLayout> compositeLayouts(context->getAmountOfSwapChainImages(),
...
VkPipeline TransparentPass::createPermutationGraphicsPipeline(const VulkanContext* context, VkPipeline basePipeline,
                                                              uint32_t flags)
{
