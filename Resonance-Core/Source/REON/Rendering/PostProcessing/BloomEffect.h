...
namespace REON {
	...
	private:
		...
		bool m_Init;
		BloomFBO m_FBO;
		uint m_ThresholdFbo, m_ThresholdTexture;
		glm::ivec2 m_SrcViewportSize;
		glm::vec2 m_SrcViewportSizeFloat;
		std::shared_ptr<Shader> m_ThresholdShader = ResourceManager::GetInstance().LoadResource<Shader>("ThresholdShader", std::make_tuple("fullScreen.vert", "Bloom/Threshold.frag", std::optional<std::string>{}));
