...
Texture::Texture(const TextureData& data)
{
    ...
    m_Texture = context->createImage(createInfo, data.pixels.data());
    ...
    vkGetPhysicalDeviceProperties(context->getPhysicalDevice(),
...
std::shared_ptr<Texture> Texture::getTextureFromId(const std::string& Id, const std::string& basePath)
{
    ...
    //		//stbi_write_png(debugPath.c_str(), header.width, header.height, header.channels, pixels.data(),
    //header.width * header.channels);

    //		tex = ResourceManager::GetInstance().LoadResource<Texture>(basePath + "\\" + texInfo->path.string(),
    //std::make_tuple(header, pixels));
    //	}
    //	else {
