...
std::vector<char> Shader::CompileHLSLToSPIRV(const std::string& source, uint32_t flags)
{
    ...
    std::filesystem::path shaderPath = std::filesystem::absolute(std::filesystem::path(source));
    std::filesystem::path parentPath = shaderPath.parent_path();

    std::wstring parentW = parentPath.wstring();

    auto filename = shaderPath.wstring();
    ...
    std::vector<LPCWSTR> arguments = {// (Optional) name of the shader file to be displayed e.g. in an error message
                                      filename.c_str(),
    ...
    if (flags & AlbedoTexture)
    {
        arguments.push_back(L"-D");
        arguments.push_back(L"USE_ALBEDO_TEXTURE");
    };
    if (flags & NormalTexture)
    {
        arguments.push_back(L"-D");
        arguments.push_back(L"USE_NORMAL_TEXTURE");
    }
    if (flags & MetallicRoughnessTexture)
    {
        arguments.push_back(L"-D");
        arguments.push_back(L"USE_METALLICROUGHNESS_TEXTURE");
    }
    if (flags & AlphaCutoff)
    {
        arguments.push_back(L"-D");
        arguments.push_back(L"ALPHA_CUTOFF");
    }
    if (flags & EmissiveTexture)
    {
        arguments.push_back(L"-D");
        arguments.push_back(L"USE_EMISSIVE_TEXTURE");
    }
    if (flags & Specular)
    {
        arguments.push_back(L"-D");
        arguments.push_back(L"USE_SPECULAR_TEXTURE");
    }
    ...
    hres = compiler->Compile(&buffer, arguments.data(), static_cast<uint32_t>(arguments.size()), includeHandler.Get(),
                             IID_PPV_ARGS(&result));
