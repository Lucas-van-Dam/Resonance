...
void Renderer::Draw(VkCommandBuffer commandBuffer, VkPipelineLayout pipelineLayout,
...
void Renderer::Draw(VkCommandBuffer commandBuffer, VkPipelineLayout pipelineLayout,
                    VkDescriptorSet perLightDescriptorSet)
{
    const VulkanContext* context = static_cast<const VulkanContext*>(Application::Get().GetRenderContext());

    std::array<VkDescriptorSet, 2> descriptorSets{perLightDescriptorSet,
                                                  shadowObjectDescriptorSets[context->getCurrentFrame()]};
...
void Renderer::Draw(glm::mat4 mainLightView, glm::mat4 mainLightProj, int skyboxId, int irradianceMapId,
                    int prefilterMapId, int brdfLUTTextureId, std::vector<int> depthCubeId, int shadowMapId,
...
Renderer::Renderer(std::shared_ptr<Mesh> mesh, std::vector<ResourceHandle> materials)
