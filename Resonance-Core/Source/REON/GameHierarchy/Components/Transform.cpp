...
void Transform::SetWorldTransform(const glm::mat4& worldTransform)
{
    ...
    glm::mat4 parentWorldTransform =
        get_owner()->GetParent() ? get_owner()->GetParent()->GetTransform()->GetWorldTransform() : glm::mat4(1.0f);
    ...
    // Extract scale
    localScale =
        glm::vec3(glm::length(localTransform[0]), glm::length(localTransform[1]), glm::length(localTransform[2]));
...
void Transform::on_component_detach() {}

void Transform::DecomposeMatrix(const std::vector<float>& matData, glm::vec3& position, glm::quat& rotation,
                                glm::vec3& scale)
{
    ...
    scale = glm::vec3(glm::length(glm::vec3(mat[0])), glm::length(glm::vec3(mat[1])), glm::length(glm::vec3(mat[2])));
...
glm::mat4 Transform::GetWorldTransform() const
{
    ...
    return (get_owner()->GetParent() ? get_owner()->GetParent()->GetTransform()->GetWorldTransform()
                                     : glm::mat4(1.0f)) *
...
Quaternion Transform::GetWorldRotation() const
{
    ...
    std::shared_ptr<GameObject> parent = get_owner()->GetParent();
