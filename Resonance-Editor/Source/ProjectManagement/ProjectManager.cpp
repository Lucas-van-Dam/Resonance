...
ProjectManager::ProjectManager() : isProjectOpen(false)
{
    ...
    serializers["float"] = [](const void* data) { return std::to_string(*static_cast<const float*>(data)); };
    ...
    RegisterDeserializer<glm::vec3>("glm::vec3",
    ...
    RegisterDeserializer<glm::uvec2>("glm::uvec2",
                                     [](const std::string& str)
                                     {
                                         glm::uvec2 result;
...
bool ProjectManager::BuildProject(const std::filesystem::path& buildDirectory)
{
    ...
    if (runtimeConfig.is_open())
    {
        ...
        configJson["WindowProperties"] = {{"width", 1200}, // Example width
                                                   ...
                                                   const nlohmann::json& sceneJson)
{
    ...
    if (objectJson.contains("Components"))
    {
        for (const auto& componentJson : objectJson["Components"])
        {
            ...
            {
                REON_CORE_WARN("Unsupported component type: {0}", componentJson["Type"].get<std::string>());
