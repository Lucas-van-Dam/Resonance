...
{
    if (!node || node->templateData->variants.empty())
        return;
...
{
    if (!node || ctx.visited.count(node))
        return;
...
std::string resolveInput(const ShaderPin& pin, CodeGenContext& ctx)
{
    if (!pin.node->inputConnections.contains(pin.templateData->name) || !pin.templateData)
        return "0.0"; // fallback
